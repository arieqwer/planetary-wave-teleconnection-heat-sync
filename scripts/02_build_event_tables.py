...
def season_suffix(season_mode: str) -> str:
    return "" if season_mode == "all" else f"_{season_mode.replace('-', '_')}"


def filter_to_local_warm_season(
    full: pd.DataFrame,
    ...
    if not season_mode.startswith("local-warm"):
        raise ValueError(f"Unsupported season mode: {season_mode}")

    n_months = int(season_mode.removeprefix("local-warm"))
    baseline = baseline.copy()
    full = full.copy()
    baseline["month"] = baseline["lt_date"].dt.month
    full["month"] = full["lt_date"].dt.month
    ...
    warm_months = monthly.loc[monthly["warm_rank"] <= n_months, spatial_cols + ["month"]]

    filtered_full = full.merge(warm_months.assign(_warm_season=1), on=spatial_cols + ["month"], how="left")
    filtered_baseline = baseline.merge(warm_months.assign(_warm_season=1), on=spatial_cols + ["month"], how="left")
    filtered_full = filtered_full.loc[filtered_full["_warm_season"] == 1].drop(columns=["_warm_season"])
    filtered_baseline = filtered_baseline.loc[filtered_baseline["_warm_season"] == 1].drop(columns=["_warm_season"])
    return filtered_full, filtered_baseline
...
def build_event_table(
    ...
    full, baseline = filter_to_local_warm_season(
        full=full,
        baseline=baseline,
        spatial_cols=spatial_cols,
        season_mode=season_mode,
    )
    if full.empty or baseline.empty:
        raise RuntimeError(f"Season mode {season_mode!r} produced no rows.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--season-mode",
        choices=["all", "local-warm3", "local-warm4", "local-warm6"],
        default="all",
        help="Restrict event detection to each node's warmest N calendar months before computing thresholds.",
    )
    parser.add_argument(
        "--urban-panel-path",
        type=Path,
...
def main() -> None:
    ...
    suffix = season_suffix(args.season_mode)
    ...
    if args.kind in {"hot", "both"}:
        jobs.append(("hot", args.output_root / f"ERA5_hotnight_events_T_Tw_dyn_2000_2024_{tag}{suffix}.parquet"))
    if args.kind in {"cold", "both"}:
        jobs.append(("cold", args.output_root / f"ERA5_coldnight_events_T_Tw_dyn_2000_2024_{tag}{suffix}.parquet"))
