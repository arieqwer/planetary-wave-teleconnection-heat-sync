...
def build_coherent_network(
    ...
    print(f"Wrote Coherent edges: {output_edges}")
    print(f"Wrote Coherent nodes: {output_nodes}")


def build_dipole_network(
    ...
    print(f"Wrote Dipole edges: {output_edges}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Coherent and Dipole event-coincidence networks.")
    parser.add_argument("--tau", type=float, default=0.15)
    parser.add_argument("--metric", choices=["T", "Tw"], default="T")
    parser.add_argument("--mode", choices=["coherent", "dipole", "both"], default="both")
    ...
    parser.add_argument(
        ...
        default="all",
        help="Use event tables generated with the matching season mode.",
    )
...
def main() -> None:
    ...
    if args.mode in {"coherent", "both"}:
        build_coherent_network(
            events_path=hot_events,
            output_edges=args.output_root / f"ECA_edges_dyn_{args.metric}{run_label}_a{alpha_label}_K{args.k_days}_{tag}{suffix}.parquet",
            output_nodes=args.output_root / f"ECA_nodes_used_{args.metric}{run_label}_{tag}{suffix}.parquet",
            metric=args.metric,
    ...
    if args.mode in {"dipole", "both"}:
        build_dipole_network(
            hot_events_path=hot_events,
            cold_events_path=cold_events,
            output_edges=args.output_root / f"ECA_edges_SEESAW_{args.metric}{run_label}_a{alpha_label}_K{args.k_days}_{tag}{suffix}.parquet",
            metric=args.metric,
            min_run_length=args.min_run_length,
            alpha=args.alpha,
            k_days=args.k_days,
